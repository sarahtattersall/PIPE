...
public class TokenSetController extends Observable {
    ...
    protected static final String NO_TOKEN_VIEW_FOUND_WITH_ID =
            "TokenSetController.setActiveTokenView: no TokenView found with ID ";
    protected static final String NEW_TOKENVIEW_LIST_HAS_CONFLICTING_OR_INVALID_ENTRIES =
            "TokenSetController.setTokenViews: new list of TokenViews has conflicting or invalid entries.  Current set not replaced.\n" +
                    "Consider making changes one at a time.";
    protected static final String NEW_TOKENVIEW_MATCHES_ID_AND_COLOR_OF_TWO_EXISTING_TOKENVIEWS =
            "TokenSetController.updateOrAddTokenView: new TokenView matches the Id of one existing TokenView and the Color of another existing TokenView, and has a different enabled status.  No updates were made.\nDetails: ";
    protected static final String CANT_DISABLE_LAST_ENABLED_TOKENVIEW =
            "TokenSetController.updateOrAddTokenView: new TokenView attempts to disable last remaining enabled TokenView.  No updates were made.";
    ...
    public void setActiveTokenView(String tokenViewId) {
        ...
        if (tokenView == null) {
            throw new IllegalArgumentException(NO_TOKEN_VIEW_FOUND_WITH_ID + tokenViewId);
        } else {
    ...
    public boolean updateOrAddTokenView(TokenView tokenView) throws TokenLockedException {
        ...
        if (onlyEnabledTokenViewIsInitialDefault) {
            return removeOrUpdateInitialDefault(tokenView);
        } else {
            return addOrUpdateMapsAndLists(tokenView);
        }
    }
    ...
    private boolean addOrUpdateMapsAndLists(TokenView tokenView) throws TokenLockedException {
        ...
        if (matchesExistingIdAndColorOfSingleTokenView(tokenView)) {
            if (sameEnabledStatus(tokenView)) {
            ...
            if (sameEnabledStatus(tokenView)) {
                ...
                throw new IllegalArgumentException(
                        NEW_TOKENVIEW_MATCHES_ID_AND_COLOR_OF_TWO_EXISTING_TOKENVIEWS + tokenView.toString());
            }
    ...
    private void changeEnabledStatus(TokenView tokenView) throws TokenLockedException {
        if (candidateStatusChangeTokenView != null) {
            ...
            if (enable) {
                ...
                if (enabledTokenViews.size() < 2) {
                    throw new IllegalArgumentException(CANT_DISABLE_LAST_ENABLED_TOKENVIEW);
                }
                ...
                if (isActive) {
                    shiftActiveToFirstEnabledTokenView();
                }
            }
        }
    }
...
//		if ((existingIdTokenView == null) && (existingColorTokenView == null)) return false;
        boolean sameColorTokenStatus =
                (existingColorTokenView != null) ? (existingColorTokenView.isEnabled() == tokenView.isEnabled()) : true;
        boolean sameIdTokenStatus =
                (existingIdTokenView != null) ? (existingIdTokenView.isEnabled() == tokenView.isEnabled()) : true;
    ...
    private void addToMapsAndLists(TokenView tokenView) {
        ...
        if (tokenView.isEnabled()) {
            enabledTokenViews.add(tokenView);
        }
    }
    ...
    private boolean matchesExistingColor(TokenView tokenView) {
        ...
        if (colorMatch) {
            candidateStatusChangeTokenView = existingColorTokenView;
        }
        return colorMatch;
    }
    ...
    protected boolean matchesExistingId() {
        ...
        if (idMatch) {
            candidateStatusChangeTokenView = existingIdTokenView;
        }
        return idMatch;
    }
    ...
    private boolean isNull(TokenView tokenView) {
        if ((tokenView == null) || (tokenView.getID() == null)) {
            return true;
        } else {
            return false;
        }
...
//			updated = false; 
    ...
    private boolean populateThisControllerOneByOne(List<TokenView> tokenViewList) throws TokenLockedException {
        ...
        if ((populated) && numberOfEnabledTokenViewsMismatch()) {
            populated = false;
        }
        return populated;
    }

    protected boolean populateControllerOneByOne(TokenSetController tokenSetController,
            List<TokenView> tokenViewList) throws TokenLockedException {
        tokenSetController.duplicateAllowed = true;
        for (TokenView tokenView : tokenViewList) {
            if (!tokenSetController.updateOrAddTokenView(tokenView)) {
                return false;
            }
        }
