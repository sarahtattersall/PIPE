...
public class StateViewer extends JLayeredPane implements Constants
{
	...
	private PetriNetView _stateDataLayer;
	private StateGroup activeStateGroup;
	ArrayList<ConditionPlaceView> _condPlaceViews;
	...
	public void drawPetriNet(PetriNetView pnmldata, StateGroup stateGroupData)
	{
		...
		_stateDataLayer = pnmldata.clone();
		...
		Iterator PNObjects = _stateDataLayer.getPetriNetObjects();
	...
	void add(PetriNetViewComponent currentObj)
	{
		...
		{
            PlaceView currentPlace = (PlaceView) currentObj;
			ConditionPlaceView placeView = new ConditionPlaceView(currentPlace, currentPlace.getPetriNetController());
			...
			if (placeCondition != null){
				if(!placeCondition.getOperator().equals("T")){
					placeView.setCondition(placeCondition.getOperator(), placeCondition.getPlaceB());
				}
				else{
					placeView.setCondition(placeCondition.getOperator(), "T");
				}
			}
