...
public class GSPNNew extends GSPN implements IModule {
            ...
            SwingWorker sw = new SwingWorker() {
                ...
                protected Object doInBackground() {
                    ...
                    if (sourceDataLayer == null) {
                        JOptionPane.showMessageDialog(null, "Please, choose a source net", "Error",
                    ...
                    for (int i = 0; i < 1; i++) {
                        if (!sourceDataLayer.hasTimedTransitions()) {
                            ...
                            try {
                                ...
                                s += "<br>Solving the steady state distribution took " + f.format(steadystatetime)
                                        + "s";
    ...
    private static void writeTable(Object[] items, int cols, boolean showLines, boolean doShading,
                                   boolean columnHeaders, boolean rowHeaders, PrintWriter out) {
        out.println("<table border=" + (showLines ? 1 : 0) + " cellspacing=2>");
        int j = 0;
        for (int i = 0; i < items.length; i++) {
            if (j == 0) {
                out.println("<tr" + (doShading ? " class=" + (i / cols % 2 == 1 ? "odd>" : "even>") : ">"));
            }
     ...
     * @return
     */
    private String displayResults(PetriNetView sourceDataLayer, File rgfile, double[] pi) {
        ...
        try {
			 ...
			 */
            out.println("<html><head>" + ResultsHTMLPane.HTML_STYLE + "</head><body>");
            out.println("<h2>GSPN Steady State Analysis Results</h2><br>");

            if (mode) {
                String states = "<br>" + "There are " + tangiblestates.size() + " tangible states. ";
                s.append(states);
                out.println(states);
                s.append("Only a summary of the results will be displayed.");
                s.append("<br> For complete results see ").append(output.getAbsolutePath());
                s.append("<br>");
            }
            ...
            if (mode) {
                ...
                String tangibles = renderTangibleStates(sourceDataLayer, tangiblestates);
                s.append(tangibles);
            ...
            if (tokendist != null) {
                String distribution = renderTokenDistribution(sourceDataLayer, tokendist);
            ...
            if (throughput != null) {
                String tThroughput = renderTimedTransitionThroughput(sourceDataLayer, throughput);
     ...
     * @return
     */
    public String renderTangibleStates(PetriNetView sourceDataLayer, StateList tangiblestates) {
        return ResultsHTMLPane.makeTable(
                new String[]{"Set of Tangible States", renderStateSpaceLinked(sourceDataLayer, tangiblestates)}, 1,
                false, false, true, false);
    }

    private void renderTangibleStates(PetriNetView sourceDataLayer, StateList tangiblestates, PrintWriter out) {
    ...
    private void renderList(double[] data, StateList list, PrintWriter out) {
        ...
        for (int i = 0; i < list.size(); i++) {
            result.add("<A HREF='#" + list.getID(i) + "'>" + list.getID(i).toUpperCase() + "</A>");
    ...
    private String renderPi(double[] pi, StateList states) {
        return ResultsHTMLPane.makeTable(
                new String[]{"Steady State Distribution of Tangible States", renderLists(pi, states)}, 1, false, false,
                true, false);
    }
     ...
     */
    public Object[] getPiAndTangibleStates(PetriNetView sourceDataLayer)
            throws OutOfMemoryError, MarkingNotIntegerException {
     ...
     * net.
     */
    public double[] getAverageTokens(PetriNetView sourceDataLayer, double pi[], StateList tangiblestates)
     ...
     * net.
     */
    public double[][] getTokenDistribution(PetriNetView sourceDataLayer, double pi[], StateList tangiblestates)
            throws NoTimedTransitionsException, OutOfMemoryError, MarkingNotIntegerException {
        File reachabilityGraph = new File("results.rg");
        if (!sourceDataLayer.hasTimedTransitions()) {
            throw new NoTimedTransitionsException();
    ...
    private double[][] tokenDistribution(double[] pi, StateList states) {
        ...
        for (int marking = 0; marking < numstates; marking++) {
            ...
            for (int p = 0; p < numplaces; p++) {
                ...
                if (current[p] > (((double[]) counter.get(p)).length - 1)) {
                    ...
                    System.arraycopy(counter.get(p), 0, copy, 0, ((double[]) counter.get(p)).length);
        ...
        for (int p = 0; p < numplaces; p++) {
            System.arraycopy(counter.get(p), 0, result[p], 0, ((double[]) counter.get(p)).length);
     ...
     * net.
     */
    public double[] getThroughput(PetriNetView sourceDataLayer, double pi[], StateList tangiblestates)
            throws NoTimedTransitionsException, OutOfMemoryError, MarkingNotIntegerException {
        File reachabilityGraph = new File("results.rg");
        if (!sourceDataLayer.hasTimedTransitions()) {
            throw new NoTimedTransitionsException();
    ...
    private double[] getFastTransitionThroughput(PetriNetView pnmldata, StateList list, double[] pi) {
        ...
        for (int marking = 0; marking < length; marking++) {
            ...
            for (int i = 0; i < markingArray.length; i++) {
                ...
                MarkingView m = new MarkingView(pnmldata.getActiveTokenView(), markingArray[i] + "");
    ...
    private String renderAverages(PetriNetView pnmldata, double[] data) {
        return ResultsHTMLPane.makeTable(new String[]{"Average Number of Tokens on a Place",
                renderLists(data, pnmldata.places(), new String[]{"Place", "Number of Tokens"})}, 1, false, false, true,
                false);
    }
     ...
     * @return
     */
    private String renderTokenDistribution(PetriNetView pnmlData, double[][] tokendist) {
        return ResultsHTMLPane.makeTable(
                new String[]{"Token Probability Density", renderProbabilityDensity(pnmlData.places(), tokendist)}, 1,
                false, false, true, false);
    }

    private String renderProbabilityDensity(PlaceView[] placeViews, double[][] probabilities) {
    ...
    private String renderTimedTransitionThroughput(PetriNetView pnmldata, double[] throughput) {
        return ResultsHTMLPane.makeTable(
                new String[]{"Throughput of Timed Transitions", renderTTThroughput(pnmldata, throughput)}, 1, false,
                false, true, false);
    }

    private String renderSojournTimes(double[] data, StateList list) {
        return ResultsHTMLPane.makeTable(new String[]{"Sojourn times for tangible states", renderLists(data, list)}, 1,
                false, false, true, false);
    }

    private void renderSojournTimes(double[] data, StateList list, PrintWriter out) {
        // add headers to table
        writeTableHeader("Sojourn times for tangible states", out);
        renderList(data, list, out);
        closeTable(out);
        ResultsHTMLPane.makeTable(new String[]{"Sojourn times for tangible states", renderLists(data, list)}, 1, false,
                false, true, false);
    }
    ...
    private double[] calcSojournTime(PetriNetView pnmldata, StateList tangibleStates) {
        ...
        for (int i = 0; i < numStates; i++) {
            ...
            for (int j = 0; j < tangibleStateArray.length; j++) {
                LinkedList<MarkingView> newMarkingViewList = new LinkedList<MarkingView>();
                MarkingView m = new MarkingView(pnmldata.getActiveTokenView(), tangibleStateArray[j]);
                newMarkingViewList.add(m);
