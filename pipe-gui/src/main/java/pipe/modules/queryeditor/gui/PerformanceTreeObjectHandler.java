...
public class PerformanceTreeObjectHandler extends MouseInputAdapter implements QueryConstants {
    ...
    public void mousePressed(final MouseEvent e) {
        if (this.enablePopup) checkForPopup(e);

        int switchCondition;
        if (MacroManager.getEditor() == null) switchCondition = QueryManager.getMode();
            ...
            case SELECT:
                if (this.isDragging) {
                    ...
                    if (this.myObject instanceof PerformanceTreeArcPathPoint) {
                        ...
                        if (MacroManager.getEditor() == null) arc = QueryManager.getView().getArcBeingModified();
                        ...
                        if (arc != null) {
                            ...
                            if (MacroManager.getEditor() == null) viewObjects = QueryManager.getView().getComponents();
                                        ...
                                        if (!nodeToLinkUpWith.getId().equals(arc.getSourceID())) {
                                            ...
                                            if (nodeToLinkUpWith.getIncomingArcID() == null) {
                                                ...
                                                if (arcConnectionValid(arc, nodeToLinkUpWith)) {
                                                    ...
                                                    // do the translation
                                                    int transX = e.getX() - this.dragInit.x;
                                                    int transY = e.getY() - this.dragInit.y;
                                                    if (MacroManager.getEditor() == null)
                                                        ((QueryView) this.contentPane).getSelectionObject().translateSelection(transX, transY);
                                                    else
                                                        ((MacroView) this.contentPane).getSelectionObject().translateSelection(transX, transY);
                                                }
                                            } else {
                                                // write info message
                                                String message = "You are trying to connect to a node that is already connected elsewhere. " + "First break the existing connection and then try connecting again.";
                                                writeErrorMessage(message);
                                            }
                                        } else {
                                            // write info message
                                            String message = "You are trying to assign the node to itself. Please drag the endpoint " + "of the arc to a valid node in order to establish a connection.";
                                            writeErrorMessage(message);
                                        }
                                    } else {
                    ...
                    if (!PerformanceTreeObjectHandler.justSelected) {
                        if (e.isShiftDown()) this.myObject.deselect();
                        else {
                            if (MacroManager.getEditor() == null)
                                ((QueryView) this.contentPane).getSelectionObject().clearSelection();
                            else ((MacroView) this.contentPane).getSelectionObject().clearSelection();
    ...
    private void sequentialNodeCase(final PerformanceTreeNode sourceNode) {
        if (sourceNode instanceof SequentialNode) {
            if (((SequentialNode) sourceNode).allArcsAssigned() && !((SequentialNode) sourceNode).haveOptionalArcAvailable()) {
    ...
    private boolean arcConnectionValid(final PerformanceTreeArc arc, final PerformanceTreeNode targetNode) {
        ...
        if (sourceNode.childAssignmentValid(arc, targetNode) || MacroManager.getEditor() != null &&
            ...
            if (momentNodeCaseChecked(arc, targetNode)) {
                ...
                String message = "The node you are trying to connect to the Moment node " + "returns a decimal value, which is invalid in this scenario. " + "Only integer values are defined for moments.";
                writeErrorMessage(message);
            }
        } else {
     ...
     * @return
     */
    private boolean momentNodeCaseChecked(final PerformanceTreeArc arc, final PerformanceTreeNode nodeToLinkUpWith) {
    ...
    private void writeErrorMessage(final String message) {
        ...
        if (MacroManager.getEditor() == null) QueryManager.writeToInfoBox(msg);
     ...
     * @return
     */
    private boolean endpointWithinNodeBounds(final PerformanceTreeArcPathPoint arcPoint, final PerformanceTreeNode node) {
    ...
    public void mouseDragged(final MouseEvent e) {
        ...
        switch (switchCondition) {
            case SELECT: {
                ...
                if (MacroManager.getEditor() == null)
                    oneSelectionObject = ((QueryView) this.contentPane).getSelectionObject().getSelectionCount() == 1;
                else oneSelectionObject = ((MacroView) this.contentPane).getSelectionObject().getSelectionCount() == 1;
                ...
                if (!this.isDragging && oneSelectionObject) {
                    if (this.myObject instanceof PerformanceTreeNode) {
                        ...
                        ((PerformanceTreeNode) this.myObject).setCentre(((PerformanceTreeNode) this.myObject).getCentre().getX(), ((PerformanceTreeNode) this.myObject).getCentre().getY());
                        ...
                        ((PerformanceTreeArcPathPoint) this.myObject).setPointLocation(((PerformanceTreeArcPathPoint) this.myObject).getPoint().getX(), ((PerformanceTreeArcPathPoint) this.myObject).getPoint().getY());

                        // clear InfoBox in case there was a message there
                        if (MacroManager.getEditor() == null) QueryManager.clearInfoBox();
...
// QueryManager.getData().printQueryDataContents();
                ...
                if (this.myObject.isDraggable()) this.isDragging = true;
    ...
    private void checkForPopup(final MouseEvent e) {
        if (myObject.enablePopup) {
            if (e.isPopupTrigger()) {
                ...
                if (m != null) m.show(this.myObject, e.getX(), e.getY());
            }
        }
    }
