...
public class InvariantAnalysis implements IModule {
    ...
    String reportPInvariants(int[] M) {
        ...
        result += ResultsHTMLPane.makeTable(_PInvariants, _pnmlData.places(), false, true, true, false);

        if (_PInvariants.isCovered()) {
            result += "The net is covered by positive P-Invariants, " + "therefore it is bounded.";
        } else {
            result +=
    ...
    String reportTInvariants(int[] M) {
        ...
        result += ResultsHTMLPane.makeTable(TInvariants, _pnmlData.getTransitionViews(), false, true, true, false);

        if (TInvariants.isCovered()) {
            result += "The net is covered by positive T-Invariants, " + "therefore it might be bounded and live.";
        } else {
