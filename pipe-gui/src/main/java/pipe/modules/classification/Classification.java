 ...
 */
public class Classification implements IModule {
            ...
            if (!sourceDataLayer.hasPlaceTransitionObjects()) {
                ...
                try {
                    s += ResultsHTMLPane.makeTable(
                            new String[]{"State Machine", "" + stateMachine(sourceDataLayer), "Marked Graph",
                                    "" + markedGraph(sourceDataLayer), "Free Choice Net",
                                    "" + freeChoiceNet(sourceDataLayer), "Extended Free Choice Net",
                                    "" + extendedFreeChoiceNet(sourceDataLayer), "Simple Net",
                                    "" + simpleNet(sourceDataLayer), "Extended Simple Net",
                                    "" + extendedSimpleNet(sourceDataLayer)}, 2, false, true, false, true);
    ...
    boolean stateMachine(PetriNetView pnmlData) throws Exception {
        ...
        for (int transitionNo = 0; transitionNo < transitionsCount; transitionNo++) {
            if ((countTransitionInputs(pnmlData, transitionNo) > 1) || (countTransitionOutputs(pnmlData, transitionNo)
                    > 1)) {
                return false;
            }
        }
    ...
    private int countPlaceOutputs(PetriNetView pnmlData, int PlaceNo) {
        int[][] forwards = pnmlData.getActiveTokenView().getBackwardsIncidenceMatrix(pnmlData.getModel().getArcs(),
                pnmlData.getModel().getTransitions(),
                pnmlData.getModel().getPlaces()); // transition backwards = place forwards
        int count = 0;
        ...
        for (int placeNo = 0; placeNo < placeCount; placeNo++) {
            for (int placeDashNo = 0; placeDashNo < placeCount; placeDashNo++) {
                if (placeDashNo != placeNo) {
                    int[] placeSet = forwardsPlaceSet(pnmlData, placeNo);
                    int[] placeDashSet = forwardsPlaceSet(pnmlData, placeDashNo);
                    if (intersectionBetweenSets(placeSet, placeDashSet) && (!isSubset(placeSet, placeDashSet))) {
                        return false;
                    }
                }
            }
    ...
    private static boolean findSubset(int[] A, int[] B) {
        ...
        for (int aA : A) {
            ...
            for (int j = 0; j < B.length; j++) {
                if (aA == B[j] && aA != -1) { // ith element of A exists in B
                    elementExists = true;
                    break;
                }
            }
